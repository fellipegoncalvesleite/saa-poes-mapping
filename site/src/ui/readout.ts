...
export function renderReadout(root: HTMLElement, configuration: Configuration, activeCell: ActiveCell | null): void {
  ...
  root.innerHTML = `<div class="metric-grid" aria-label="Current canonical metrics">
    ...
    <div><dt>Coverage</dt><dd>${activeCell.covered ? "Passed" : "Failed"}</dd></div><div><dt>Footprint</dt><dd>${activeCell.selected ? "Selected" : "Not selected"}</dd></div>
    <div><dt>Approx. cell span</dt><dd>${number(activeCell.northSouthKm, 0)} km N–S × ${number(activeCell.eastWestKm, 0)} km E–W</dd></div><div><dt>Physical cell area</dt><dd>${number(activeCell.areaKm2, 0)} km²</dd></div></dl>` : "<p>Point, tap, or focus the map and use arrow keys to inspect a cell. Physical size is reported for the inspected latitude.</p>"}</div>`;
}
